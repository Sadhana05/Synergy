...
const SignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Sanitize username: remove spaces, lowercase, append random suffix to avoid duplicates
    const sanitizedUsername = trimmedName.replace(/\s+/g, '').toLowerCase() + '_' + Math.random().toString(36).substring(2, 7);

    try {
      setIsSubmitting(true);
      await register(trimmedEmail, sanitizedUsername, trimmedPassword, trimmedName);
        ...
        <Card className="border-border/50 bg-card/60 backdrop-blur-xl shadow-2xl">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
              ...
              <Button
                type="submit"
                className="w-full flex items-center justify-center rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90 hover:shadow-lg hover:shadow-primary/20 transition-all mt-4"
